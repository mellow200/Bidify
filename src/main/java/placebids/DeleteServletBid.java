package placebids;


import java.io.IOException;
import java.util.List;

import javax.servlet.RequestDispatcher;
import javax.servlet.ServletException;
import javax.servlet.annotation.WebServlet;
import javax.servlet.http.HttpServlet;
import javax.servlet.http.HttpServletRequest;
import javax.servlet.http.HttpServletResponse;


@WebServlet("/DeleteServletBid")
public class DeleteServletBid extends HttpServlet {
	private static final long serialVersionUID = 1L;
 

	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {

	}

	
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		String id = request.getParameter("bid");
		boolean isTrue;
		
		isTrue = BidController.deletedata(id);
		
		if(isTrue==true) {
		String alertMessage = "Data Deletion Successful";
		response.getWriter().println("<script> alert('"+alertMessage+"'); window.location.href='BidViewServlet'</script>");
		}
		else {
			List<BidModel> biddetails = BidController.getById(id);
			request.setAttribute("biddetails", biddetails);
			
			RequestDispatcher dispatcher = request.getRequestDispatcher("Done.jsp");
			dispatcher.forward(request, response);
			
		}
	}

}
